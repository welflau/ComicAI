...
| 节点 | 说明 |
|------|------|
| `libtv_script` | 文本节点：剧本编写、MD 文件导入、角色图提取 |
| `libtv_script_gen` | 分镜脚本：AI 生成分镜表 + 批量创建配图节点 |
| `libtv_image` | 图片节点：文生图（Lib Nano Pro / 即梦）、图生图 |
| `libtv_video` | 视频节点：文生视频 / 图生视频（可灵 / 即梦） |
| `libtv_video_compose` | 视频合成：多段拼接 + 转场（FFmpeg） |
| `libtv_chapter_split` | 章节分解：小说自动按章节拆分，生成章节节点组 |
| `libtv_group` | 节点组：将多个节点打包，双击进入子画布 |
| `libtv_loop` | 循环遍历：逐一推送组内节点内容，支持一键遍历全部 |
| `libtv_log` | Log 节点：读取上游节点输出，调试用 |
